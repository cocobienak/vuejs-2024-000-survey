        ...
        <CForm>
          ...
          <CRow>
            ...
            <CCol sm="6">
              <CInput
                name="stories_name"
                v-model="fieldForm.stories_name.value"
                placeholder="Code domain ..."
                :invalid-feedback="globalFunctions.getInvalidFeedback(this, 'fieldForm', 'stories_name')"
                :is-valid="globalFunctions.getIsValidfield(this, 'fieldForm', 'stories_name')"
                @input="globalFunctions.processCheckFieldForm(vm, 'fieldForm', 'stories_name')"                
                label="Stories"
              />
            </CCol>
          </CRow> 
...
export default {
  ...
  created () {
    if (this.$route.params.id) {
      this.processGetDomainMessageStoriesDetail();
    }
  },
  mounted() {
  },
  methods: {
    processGetDomainMessageStoriesDetail: function () {
      const data = {
      };
      baseCallApiHelper.callApi(
        "Get domain group detail",
        null,
        api.domain_message_stories.read.method,
        api.domain_message_stories.read.url + '/' + this.$route.params.id, 
        data, 
        (status, data, message) => {
          if(status) {
            ...
            this.fieldForm.stories_name.value = data.data["stories_id.name"];
