    ...
    <CCard>
      <CCardBody>
        <CButton
          ...
          square
          size="sm"
          @click="$router.push({path: '/system-user/add'})"
        >
          Add new
        </CButton>
        <hr/>
          ...
          <template #edit_button="{item}">
            <td class="py-2">
              <CButton
                ...
                square
                size="sm"
                @click="$router.push({ path: `/system-user/edit/${item.id}` })"
              >
                Edit
              </CButton>
            </td>
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.togglePrevious100k(vm, vm.processGetSystemUserList)"
        >
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.toggleNext100k(vm, vm.processGetSystemUserList)"
        >
          Next 100k record
        </CButton>
      </CCardBody>
...
export default {
  ...
  data () {
    return {
      ...
      // isField = true, for include into query to api
        ...
        { key: 'username', label: 'Username', _style:'min-width:200px', isFields: true },
      ...
      // Classes d-none & sr_only make visibility hide on table
        { key: 'entity_detail_id', _classes:["d-none", "sr_only"], isFields: true },
        { key: 'entity_detail_id.id_ktp', _classes:["d-none", "sr_only"], isFields: true },
        { key: 'entity_detail_id.id_passport', _classes:["d-none", "sr_only"], isFields: true },
        { key: 'entity_detail_id.address_1', _classes:["d-none", "sr_only"], isFields: true },
        { key: 'entity_detail_id.address_2', _classes:["d-none", "sr_only"], isFields: true },
  ...
  mounted() {
    this.processGetSystemUserList();
  },
  methods: {
    processGetSystemUserList: function (e) {
        const data = {
        ...
        baseCallApiHelper.callApi(
          ...
          null,
          api.system_user.list.method,
          api.system_user.list.url, 
          data, 
          (status, data, message) => {
            if(status) {
    ...
    processDeleteSystemUser: function () {
      const data = {
      };      
      if (this.itemDeleteID) {
        baseCallApiHelper.callApi(
          "Delete System User",
          null,
          api.system_user.delete.method,
          api.system_user.delete.url + '/' + this.itemDeleteID, 
          data, 
          (status, data, message) => {
