    ...
    <CCard>
      <CCardBody>
          ...
          <template #edit_button="{item}">
            <td class="py-2">
              <CButton
                ...
                square
                size="sm"
                @click="$router.push({ path: `/domain-message/edit/${item.id}` })"
              >
                View
              </CButton>
            </td>
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.togglePrevious100k(vm, vm.processGetDomainMessageList)"
        >
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.toggleNext100k(vm, vm.processGetDomainMessageList)"
        >
          Next 100k record
        </CButton>
      </CCardBody>
      ...
      <div slot="footer-wrapper" class="w-100">
        <div class="modal-footer">
          <CButton
            ...
            square
            size="sm"
            @click="processDeleteDomainMessage()"
          >
            Delete
          </CButton>
          <CButton
...
export default {
  ...
  mounted() {
    this.processGetDomainMessageList();
  },
  methods: {
    processGetDomainMessageList: function (e) {
        const data = {
        ...
        baseCallApiHelper.callApi(
          "Get domain message list",
          null,
          api.domain_message.list.method,
          api.domain_message.list.url, 
          data, 
          (status, data, message) => {
            if(status) {
