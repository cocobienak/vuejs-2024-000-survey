    ...
    <CCard>
      <CCardBody>
        <CButton
          ...
          square
          size="sm"
          @click="$router.push({path: '/event-category/add'})"
        >
          Add new
        </CButton>
        <hr/>
          ...
          <template #edit_button="{item}">
            <td class="py-2">
              <CButton
                ...
                square
                size="sm"
                @click="$router.push({ path: `/event-category/edit/${item.id}` })"
              >
                Edit
              </CButton>
            </td>
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.togglePrevious100k(vm, vm.processGetEventCategoryList)"
        >
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.toggleNext100k(vm, vm.processGetEventCategoryList)"
        >
          Next 100k record
        </CButton>
      </CCardBody>
      ...
      <div slot="footer-wrapper" class="w-100">
        <div class="modal-footer">
          <CButton
            ...
            square
            size="sm"
            @click="processDeleteEventCategory()"
          >
            Delete
          </CButton>
          <CButton
...
export default {
  ...
  methods: {
    processGetEventCategoryList: function (e) {
        const data = {
    ...
    processDeleteEventCategory: function () {
      const data = {
      };      
      if (this.itemDeleteID) {
        baseCallApiHelper.callApi(
          ...
          null,
          api.event_category.delete.method,
          api.event_category.delete.url + '/' + this.itemDeleteID, 
          data, 
          (status, data, message) => {
