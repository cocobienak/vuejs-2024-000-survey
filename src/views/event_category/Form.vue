<template>
  <div>
    <CCard>
      <CCardBody>
        <CForm
          @submit.prevent="processFormEventCategory"
        >
          ...
          <div style="float:right" v-if="(fieldForm.id.value !== null && fieldForm.id.value !== '')">
              <CButton
                ...
                square
                size="sm"
                @click="globalFunctions.reloadAddNew($router, 'event-category/add')"
              >
              Add new
            </CButton>
          </div>
          ...
          <CRow>
            <CCol sm="6">
                ...
                placeholder="Name Event Category ..."
...
export default {
  ...
  created () {
    if (this.$route.params.id) {
      ...
      this.processGetEventCategoryDetail();
    } else {
  ...
  methods: {
    processGetEventCategoryDetail: function () {
      const data = {
      };
      baseCallApiHelper.callApi(
        "Get event category detail",
        null,
        api.event_category.read.method,
        api.event_category.read.url + '/' + this.$route.params.id, 
        data, 
        (status, data, message) => {
          if(status) {
    ...
    processFormEventCategory: function (e) {
      if (globalFunctions.validationForm(this, 'fieldForm')) {
        ...
        if (this.fieldForm.id.value === null) {
          ...
          baseCallApiHelper.callApi(
            ...
            null,
            api.event_category.create.method,
            api.event_category.create.url, 
            data, 
            (status, data, message) => {
              if(status) {
                this.fieldForm.id.value = data.data.id
          ...
          baseCallApiHelper.callApi(
            ...
            null,
            api.event_category.update.method,
            api.event_category.update.url,  
            data, 
            (status, data, message) => {
              if(status) {
    ...
    processRefresh(id){
      router.push({ path: `/event-category/edit/${id}` });
    ...
    processGetTree : function () {
      ...
      baseCallApiHelper.callApi(
        "Get tree list",
        null,
        api.event_category.getTree.method,
        api.event_category.getTree.url + '/' + this.$route.params.id, 
        data, 
        (status, data, message) => {
          if(status) {
    ...
    processGetParentList : function () {
      ...
      baseCallApiHelper.callApi(
        "Get parent list",
        null,
        api.event_category.listParent.method,
        api.event_category.listParent.url + '/' + this.$route.params.id, 
        data, 
        (status, data, message) => {
          if(status) {
