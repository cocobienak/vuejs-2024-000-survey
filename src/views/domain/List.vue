    ...
    <CCard>
      <CCardBody>
        <CButton
          ...
          square
          size="sm"
          @click="$router.push({path: '/domain/add'})"
        >
          Add new
        </CButton>
        <hr/>
          ...
          <template #edit_button="{item}">
            <td class="py-2">
              <CButton
                ...
                square
                size="sm"
                @click="$router.push({ path: `/domain/edit/${item.id}` })"
              >
                Edit
              </CButton>
            </td>
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.togglePrevious100k(vm, vm.processGetDomainList)"
        >
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.toggleNext100k(vm, vm.processGetDomainList)"
        >
          Next 100k record
        </CButton>
      </CCardBody>
...
export default {
  ...
  data () {
    return {
      ...
      fields : [
        ...
        { key: 'code', label: 'Code Domain', _style:'min-width:100px', isFields: true },
        { key: 'name', label: 'Name Domain', _style:'min-width:200px', isFields: true },
        { key: 'email', label: 'Email', _style:'min-width:100px', isFields: true },
        { key: 'email_cc', label: 'CC < Email >', _classes:["d-none", "sr_only"], isFields: true },
        { key: 'phone_number', label: 'Phone Number', _style:'min-width:100px', isFields: true },
  ...
  mounted() {
    this.processGetDomainList();
  },
  methods: {
    processGetDomainList: function (e) {
        const data = {
    ...
    processDeleteDomain: function () {
      ...
      if (this.itemDeleteID) {
        baseCallApiHelper.callApi(
          "Delete domain",
          null,
          api.domain.delete.method,
          api.domain.delete.url + '/' + this.itemDeleteID, 
          data, 
          (status, data, message) => {
