    ...
    <CCard>
      <CCardBody>
        <CButton
          ...
          square
          size="sm"
          @click="$router.push({path: '/stories-author/add'})"
        >
          Add new
        </CButton>
        <hr/>
          ...
          <template #edit_button="{item}">
            <td class="py-2">
              <CButton
                ...
                square
                size="sm"
                @click="$router.push({ path: `/stories-author/edit/${item.id}` })"
              >
                Edit
              </CButton>
            </td>
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.togglePrevious100k(vm, vm.processGetStoriesAuthorList)"
        >
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.toggleNext100k(vm, vm.processGetStoriesAuthorList)"
        >
          Next 100k record
        </CButton>
      </CCardBody>
      ...
      <div slot="footer-wrapper" class="w-100">
        <div class="modal-footer">
          <CButton
            ...
            square
            size="sm"
            @click="processDeleteStoriesAuthor()"
          >
            Delete
          </CButton>
          <CButton
...
export default {
  ...
  data () {
    return {
      ...
      fields : [
        ...
        { key: 'prefix_degree', label: 'Prefix Title', _style:'min-width:100px', isFields: true },
        { key: 'name', label: 'name', _style:'min-width:100px', isFields: true },
        { key: 'sufix_degree', label: 'Sufix Title', _style:'min-width:100px', isFields: true },
        { key: 'email', label: 'Description', _style:'min-width:200px', isFields: true },
  ...
  methods: {
    processGetStoriesAuthorList: function (e) {
        const data = {
    ...
    processDeleteStoriesAuthor: function () {
      const data = {
      };      
      if (this.itemDeleteID) {
        baseCallApiHelper.callApi(
          "Delete stories author",
          null,
          api.stories_author.delete.method,
          api.stories_author.delete.url + '/' + this.itemDeleteID, 
          data, 
          (status, data, message) => {
