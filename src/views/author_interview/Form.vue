    ...
    <CCard>
      <CCardBody>
        ...
        <CForm style="clear:both"
          @submit.prevent="processFormAuthorInterview" v-if="showMainForm"
        >
          ...
          <div style="float:right" v-if="(fieldForm.id.value !== null && fieldForm.id.value !== '')">
              <CButton
                ...
                square
                size="sm"
                @click="globalFunctions.reloadAddNew($router, 'author-interview/add')"
              >
              Add new
            </CButton>
          </div>
          ...
          <CRow>
            <CCol sm="6">
              <CInput
                ...
                placeholder="Name Author Interview ..."
            ...
            <CCol sm="6">
              ...
              <input type="radio" id="publish_2" value="publish_global" v-model="fieldForm.publish_status_form.value"
                style="margin-right:15px;cursor:pointer;">
              <label for="publish_2" style="cursor:pointer;">PUBLISH ON Author Interview</label>
              <br>
                <label class="text-info">
                  (this option will need approval from Author Interview admin)
                </label>
              <br>
              <label style ="width: 100%;margin-top: 0.25rem;font-size: 80%;color: #e55353;" 
          ...
          <CRow>
            <CCol sm="12">
              <div>
                <label>Author Interview</label>&nbsp;&nbsp;              
                <CButton variant="outline" color="info" square size="sm" @click="fieldForm.author_interview_html.showHTML = false">HTML Code</CButton>&nbsp;&nbsp;&nbsp;
                <CButton variant="outline" color="info" square size="sm" @click="fieldForm.author_interview_html.showHTML = true">Visual</CButton> :
              </div>
              <div v-if="fieldForm.author_interview_html.showHTML">
                <div id="editor-toolbar-author_interview_html">
                ...
                <vue-editor
                  ref="fieldForm_author_interview_html" 
                  v-model="fieldForm.author_interview_html.value" 
                  :editorOptions="fieldForm.author_interview_html.editorSettings"
                  :customModules="fieldForm.author_interview_html.modulesForEditor"
                  @input="globalFunctions.processCheckFieldForm(vm, 'fieldForm', 'author_interview_html')"
                >
                </vue-editor>
                <label style ="width: 100%;margin-top: 0.25rem;font-size: 80%;color: #e55353;" 
                  v-show="(fieldForm.author_interview_html.isValid === null) ? false : !fieldForm.author_interview_html.isValid">
                    {{globalFunctions.getInvalidFeedback(this, 'fieldForm', 'author_interview_html')}}
                </label>
              </div>
              <CTextarea 
                v-if="!fieldForm.author_interview_html.showHTML"
                rows="10"
                name="description"
                v-model="fieldForm.author_interview_html.value"
                placeholder="Description ..."
                :invalid-feedback="globalFunctions.getInvalidFeedback(this, 'fieldForm', 'author_interview_html')"
                :is-valid="globalFunctions.getIsValidfield(this,'fieldForm', 'author_interview_html')"
                @input="globalFunctions.processCheckFieldForm(vm, 'fieldForm', 'author_interview_html')"
          ...
          <CRow v-if="(fieldForm.id.value !== null && fieldForm.id.value !== '')">
            <CCol sm="12">
              <div>
                ...
                <multiselect 
                  v-model="fieldFormShare.author_interview_share_domain.value" 
                  :options="fieldFormShare.author_interview_share_domain.options" 
                  :custom-label="optionsDomain" 
                  :allow-empty="true"
                  :multiple="true"
                  placeholder="Select Domain To Share"
                  @input="globalFunctions.processCheckFieldForm(vm, 'fieldFormShare', 'author_interview_share_domain')"                 
                  label="code" track-by="name">
                </multiselect>
                <label style ="width: 100%;margin-top: 0.25rem;font-size: 80%;color: #e55353;" 
                  v-show="(fieldFormShare.author_interview_share_domain.isValid === null) ? false : !fieldFormShare.author_interview_share_domain.isValid">
                    {{globalFunctions.getInvalidFeedback(this, 'fieldFormShare', 'author_interview_share_domain')}}
                  </label>
              </div> 
            </CCol>
          </CRow>         
...
export default {
  ...
  created () {
    ...
    if (this.$route.params.id) {
      ...
      this.processGetAuthorInterviewDetail();
  ...
  methods: {
    processGetAuthorInterviewDetail: function () {
      const data = {
      };
      baseCallApiHelper.callApi(
        "Get author interview  detail",
        null,
        api.author_interview.read.method,
        api.author_interview.read.url + '/' + this.$route.params.id + '?is_form=1', 
        data, 
        (status, data, message) => {
          if(status) {
            ...
            // Share Domain ------------------------
              ...
              this.fieldFormShare.author_interview_share_domain.tempValue = data.author_interview_share_domain;

              var valueShareToDomain = [];
              var optionsShareToDomain = this.fieldFormShare.author_interview_share_domain.options;
              data.author_interview_share_domain.forEach(function(item) {
                valueShareToDomain.push(optionsShareToDomain.find(element => element.id === item.domain_id));
              });
              this.fieldFormShare.author_interview_share_domain.value = valueShareToDomain;
            // ------------------------ Share Domain
    ...
    processFormAuthorInterview: function (e) {
      if (globalFunctions.validationForm(this, 'fieldForm')) {
        ...
        if (this.fieldForm.id.value === null) {
          ...
          if (allowForSuAdmin) {
            baseCallApiHelper.callApi(
              "Create author interview ",
              null,
              api.author_interview.create.method,
              api.author_interview.create.url, 
              data, 
              (status, data, message) => {
                if(status) {
                  console.log(data);
          ...
          baseCallApiHelper.callApi(
            ...
            null,
            api.author_interview.update.method,
            api.author_interview.update.url,  
            data, 
            (status, data, message) => {
              if(status) {
    ...
    processFormCode: function () {
      if (globalFunctions.validationForm(this, 'fieldFormCode') && (this.fieldForm.id.value !== null && this.fieldForm.id.value !== '')) {
        const data = {
          ...
          baseCallApiHelper.callApi(
            "Update slug author interview",
            null,
            api.author_interview.updatecode.method,
            api.author_interview.updatecode.url,  
            data, 
            (status, data, message) => {
              if(status) {
                this.fieldFormCode.code.value = data.data.code;
    ...
    processGetListDomainForShare: function () {     
      ...
      baseCallApiHelper.callApi(
        ...
        {"Privilege": "readAuthorInterview"},
        api.domain.list.method,
        api.domain.list.url, 
        data, 
        (status, data, message) => {
          if(status) {
          this.fieldFormShare.author_interview_share_domain.options = data.data;

          if (this.fieldFormShare.author_interview_share_domain.tempValue !== "") {
            var valueShareToDomain = [];
            var optionsShareToDomain = this.fieldFormShare.author_interview_share_domain.options;
            this.fieldFormShare.author_interview_share_domain.tempValue.forEach(function(item) {
              valueShareToDomain.push(optionsShareToDomain.find(element => element.id === item.domain_id));
            });
            this.fieldFormShare.author_interview_share_domain.value = valueShareToDomain; 
          }
    ...
    processSaveShareToDomain: function() {
      if (this.fieldForm.id.value != null && this.fieldForm.id.value != '') {
        const data = {
            author_interview_id  : this.fieldForm.id.value,
            author_interview_share_domain     : this.fieldFormShare.author_interview_share_domain.value,
        }; 

          baseCallApiHelper.callApi(
            "Share author interview",
            null,
            api.author_interview.updateshareauthorinterview.method,
            api.author_interview.updateshareauthorinterview.url, 
            data, 
            (status, data, message) => {
              if(status) {
              } 
