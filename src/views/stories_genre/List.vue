    ...
    <CCard>
      <CCardBody>
        <CButton
          ...
          square
          size="sm"
          @click="$router.push({path: '/stories-genre/add'})"
        >
          Add new
        </CButton>
        <hr/>
          ...
          <template #edit_button="{item}">
            <td class="py-2">
              <CButton
                ...
                square
                size="sm"
                @click="$router.push({ path: `/stories-genre/edit/${item.id}` })"
              >
                Edit
              </CButton>
            </td>
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.togglePrevious100k(vm, vm.processGetStoriesGenreList)"
        >
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.toggleNext100k(vm, vm.processGetStoriesGenreList)"
        >
          Next 100k record
        </CButton>
      </CCardBody>
      ...
      <div slot="footer-wrapper" class="w-100">
        <div class="modal-footer">
          <CButton
            ...
            square
            size="sm"
            @click="processDeleteStoriesGenre()"
          >
            Delete
          </CButton>
          <CButton
...
export default {
  ...
  data () {
    return {
      ...
      fields : [
        ...
        { key: 'parent_id.name', label: 'Parent', _style:'min-width:100px', isFields: true },
        { key: 'parent_id', label: 'Parent', _classes:["d-none", "sr_only"], isFields: true },
  ...
  methods: {
    processGetStoriesGenreList: function (e) {
        const data = {
    ...
    processDeleteStoriesGenre: function () {
      const data = {
      };      
      if (this.itemDeleteID) {
        baseCallApiHelper.callApi(
          "Delete stories genre",
          null,
          api.stories_genre.delete.method,
          api.stories_genre.delete.url + '/' + this.itemDeleteID, 
          data, 
          (status, data, message) => {
