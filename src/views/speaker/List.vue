    ...
    <CCard>
      <CCardBody>
        <CButton
          ...
          square
          size="sm"
          @click="$router.push({path: '/speaker/add'})"
        >
          Add new
        </CButton>
        <hr/>
          ...
          <template #edit_button="{item}">
            <td class="py-2">
              <CButton
                ...
                square
                size="sm"
                @click="$router.push({ path: `/speaker/edit/${item.id}` })"
              >
                Edit
              </CButton>
            </td>
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.togglePrevious100k(vm, vm.processGetSpeakerList)"
        >
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.toggleNext100k(vm, vm.processGetSpeakerList)"
        >
          Next 100k record
        </CButton>
      </CCardBody>
...
export default {
  ...
  methods: {
    processGetSpeakerList: function (e) {
        const data = {
    ...
    processDeleteSpeaker: function () {
      ...
      if (this.itemDeleteID) {
        baseCallApiHelper.callApi(
          "Delete speaker",
          null,
          api.speaker.delete.method,
          api.speaker.delete.url + '/' + this.itemDeleteID, 
          data, 
          (status, data, message) => {
