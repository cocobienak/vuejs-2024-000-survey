    ...
    <CCard>
      <CCardBody>
        <CButton
          ...
          square
          size="sm"
          @click="$router.push({path: '/domain-group/add'})"
        >
          Add new
        </CButton>
        <hr/>
          ...
          <template #edit_button="{item}">
            <td class="py-2">
              <CButton
                ...
                square
                size="sm"
                @click="$router.push({ path: `/domain-group/edit/${item.id}` })"
              >
                Edit
              </CButton>
            </td>
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.togglePrevious100k(vm, vm.processGetDomainGroupList)"
        >
        ...
        <CButton
          ...
          square
          size="sm"
          @click="globalFunctions.toggleNext100k(vm, vm.processGetDomainGroupList)"
        >
          Next 100k record
        </CButton>
      </CCardBody>
      ...
      <div slot="footer-wrapper" class="w-100">
        <div class="modal-footer">
          <CButton
            ...
            square
            size="sm"
            @click="processDeleteDomainGroup()"
          >
            Delete
          </CButton>
          <CButton
...
export default {
  ...
  data () {
    return {
      ...
      fields : [
        ...
        { key: 'domain_id.code', label: 'Parent Domain', _style:'min-width:100px;', isFields: true },
        { key: 'domain_child_id', label: 'Child Domain', _style:'min-width:200px', isFields: true },
  ...
  mounted() {
    this.processGetDomainGroupList();
  },
  methods: {
    processGetDomainGroupList: function (e) {
        const data = {
        ...
        baseCallApiHelper.callApi(
          ...
          null,
          api.domain_group.list.method,
          api.domain_group.list.url, 
          data, 
          (status, data, message) => {
            if(status) {
    ...
    processDeleteDomainGroup: function () {
      const data = {
      };      
      if (this.itemDeleteID) {
        baseCallApiHelper.callApi(
          "Delete domain group",
          null,
          api.domain_group.delete.method,
          api.domain_group.delete.url + '/' + this.itemDeleteID, 
          data, 
          (status, data, message) => {
